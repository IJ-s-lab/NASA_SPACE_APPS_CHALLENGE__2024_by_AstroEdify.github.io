...
function scaleHandler() {
    ...
    $(document).on('mousewheel DOMMouseScroll', function (event) {
        ...
        $('.star').css('transform', 'scale(' + 1 / scale + ')');
    });
}
    ...
    start.click();
        ...
        /*function createStars() {
            ...
            for (let i = 0; i < 500; i++) {
              ...
              star.style.top = Math.random() * 100 + "%";
              star.style.left = Math.random() * 100 + "%";
              star.style.backgroundImage = "url('yellowstar.png')";
              yellowStars.push(star);
              container.appendChild(star);
            }
          
            // Create red stars to fill the gaps
            for (let i = 0; i < 500; i++) {
              const star = document.createElement("div");
              star.className = "star";
              star.style.width = "1px";
              ...
              } while (isNearYellowStar(top, left, yellowStars));
              star.style.top = top;
              star.style.left = left;
              star.style.backgroundImage = "url('redstar.png')" ;
              redStars.push(star);
              container.appendChild(star);
            }
          }
          
          function isNearYellowStar(top, left, yellowStars) {
            for (const yellowStar of yellowStars) {
              const yellowStarTop = parseFloat(yellowStar.style.top);
              const yellowStarLeft = parseFloat(yellowStar.style.left);
              if (Math.abs(parseFloat(top) - yellowStarTop) < 5 && Math.abs(parseFloat(left) - yellowStarLeft) < 5) {
                return true;
              }
            }
            return false;
